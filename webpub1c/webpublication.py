...
class WebPublication:
    ...
    @staticmethod
    def from_config(
        name: str, config: str, templates_env: Optional[jinja2.Environment] = None
    ) -> "WebPublication":
        ...
        return WebPublication(
            name, _dir, _vrd, _url, templates_env=templates_env, infobase_filepath=_file
        )
    ...
    def is_valid(self) -> bool:
        ...
        return (
            is_valid_filepath(self.directory, platform="auto")
            and is_valid_filepath(self.vrd_filename, platform="auto")
            and is_valid_filepath(self.url_path, platform="posix")
            and self.url_path.startswith("/")
            and "" != self.name
        )
    ...
    def is_ok_to_create(self) -> bool:
        ...
        return self.is_valid() and not (
            os.path.exists(self.directory) or os.path.exists(self.vrd_filename)
        )

    def is_exist(self) -> bool:
        """directory and vrd file exists"""

        return os.path.isdir(self.directory) and os.path.exists(self.vrd_filename)
